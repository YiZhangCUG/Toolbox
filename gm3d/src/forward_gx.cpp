...
int GM3D::ForwardGx(char* noise_level){
	int i,j;
...
#pragma omp parallel for private(j,R222,R122,R212,R112,R221,R121,R211,R111,G222,G122,G212,G112,G221,G121,G211,G111,x1,x2,y1,y2,z1,z2) shared(i) schedule(guided)
		for (j = 0; j < model_num_; j++){
			if (fabs(forward_model_[j]) > ZERO){
				...
				G222=log((y2-grav_obs_[i].y)+R222);
				G122=log((y2-grav_obs_[i].y)+R122);
				G212=log((y1-grav_obs_[i].y)+R212);
				G112=log((y1-grav_obs_[i].y)+R112);
				G221=log((y2-grav_obs_[i].y)+R221);
				G121=log((y2-grav_obs_[i].y)+R121);
				G211=log((y1-grav_obs_[i].y)+R211);
				G111=log((y1-grav_obs_[i].y)+R111);
